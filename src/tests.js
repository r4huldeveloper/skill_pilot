...
const Tests = {
  run() {
    console.group('🚀 Running SkillPilot Sovereign Tests...');
    try {
      this.testSanitize();
      this.testParsers();
  ...
  testParsers() {
    ...
    this.assert(skills[0].name === 'JS', 'Prompts.parseSkills works');

    // Score Parser fallback
...
export const RunTests = () => Tests.run();
